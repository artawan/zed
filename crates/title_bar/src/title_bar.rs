...
>>>>>>> Toggle titlebar options
=======
        if workspace::WorkspaceSettings::get_global(cx).show_top_bar || !cx.is_fullscreen() {
            h_flex()
                .id("titlebar")
                .w_full()
                .h(height)
...
>>>>>>> [ADD] Add options to hide topbar when enter fullscreen or configuration show_top_bar is false
    }
}
